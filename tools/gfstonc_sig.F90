...
subroutine read_nemsio_header_sig(filename,nlons,nlats,nlevs,idate,nfhour)
   ...
   call nemsio_getfilehead(gfile,dimx=nlons,dimy=nlats,dimz=nlevs,idate=idate,nfhour=nfhour,iret=iret)
   if (iret/=0) then
      write(6,*)'problem with nemsio_getheadvar, iret=',iret
      stop
   end if
   call nemsio_close(gfile, iret=iret)
end subroutine read_nemsio_header_sig
